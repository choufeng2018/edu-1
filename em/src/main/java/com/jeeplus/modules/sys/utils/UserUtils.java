...
public class UserUtils {
	...
	public static User get(String id){
		...
		if (user ==  null){
			...
			Role roleT = new Role(user);
			roleT.setSchoolCode(user.getSchool().getSchoolCode());
			user.setRoleList(roleDao.findList(roleT));
	...
	public static User getUserByMobile(String mobile){
		...
		if (user == null){
			...
			//user.setRoleList(roleDao.findList(new Role(user)));
			//CacheUtils.put(USER_CACHE, USER_CACHE_ID_ + user.getId(), user);
			CacheUtils.put(USER_CACHE, USER_CACHE_LOGIN_NAME_ + mobile, user);
		}
		return user;
	}
	 ...
	 * @return 
	 */
	public static boolean getUserIsParent(){ 
		return getUser().getCompany().getParentId().equals("0");
	}
	 ...
	 * @return 
	 */
	public static String getUserParentId(){
		String tag = getUser().getCompany().getParentId();
	...
	public static List<Role> getRoleList(){
		...
		if (roleList == null){
			if (user.isAdmin() || user.isSuperAdmin()){
				Role role = new Role();
				role.setSchoolCode(user.getSchool().getSchoolCode());
	...
	public static List<Menu> getMenuList(){
		...
		if (menuList == null){
			if (user.isAdmin()){ // 系统超级管理员
				menuList = menuDao.findAllList(new Menu());
			} else if (user.isSuperAdmin()){ // 租户的超级管理员
				Menu m = new Menu();
				m.setSchoolCode(user.getSchool().getSchoolCode());
				menuList = menuDao.findAllListBySchoolCode(m);
			} else {
				Menu m = new Menu(); // 一般用户
	 ...
	 * @return
	 */
	public static List<Menu> getMenuListByResource(String schoolcode){
	...
	public static List<Office> getOfficeList(){
		User user = getUser();// 
		@SuppressWarnings("unchecked")
		List<Office> officeList = (List<Office>)getCache(CACHE_OFFICE_LIST+UserUtils.getUser().getSchool().getSchoolCode());
		if (officeList == null){
			if (user.isAdmin() || user.isSuperAdmin()){
				...
				if (!UserUtils.getUser().getCompany().getParentId().equals("0")) {
					String str = " and a.id in ('"+UserUtils.getUser().getCompany().getId()+"', '"+UserUtils.getUser().getCompany().getParentId()+"')";
					//office.getSqlMap().put("dsf", BaseService.dataScopeFilter(user, "a", ""));
					office.getSqlMap().put("dsf", str);
				}
				officeList = officeDao.findList(office);
			}
			putCache(CACHE_OFFICE_LIST+UserUtils.getUser().getSchool().getSchoolCode(), officeList);
		}
		return officeList;
	}
	...
	public static List<Office> getOfficeAllList(){
		@SuppressWarnings("unchecked")
		List<Office> officeList = (List<Office>)getCache(CACHE_OFFICE_ALL_LIST+UserUtils.getUser().getSchool().getSchoolCode());
		if (officeList == null){
			Office office = new Office();
			office.setSchoolCode(getUser().getSchool().getSchoolCode());
			officeList = officeDao.findAllList(office);
		}
		return officeList;
	}
