...
public class UserController extends BaseController {
	...
	public String list(String officeid, String officename, User user, HttpServletRequest request, HttpServletResponse response, Model model) {
		...
		if (user.getSchool()==null || user.getSchool().getId()==null || "".equals(user.getSchool().getId()))
			user.setSchool(UserUtils.getUser().getSchool());
		if (!UserUtils.getUser().getCompany().getParentId().equals("0"))
			user.setCompany(UserUtils.getUser().getCompany());
//		if (user.getOffice()!=null && !"".equals(user.getOffice().getName()) && !"".equals(user.getOffice().getId()))
//			user.setOffice(officeService.get(user.getOffice().getId()));
	...
	public String list1(User user, HttpServletRequest request, HttpServletResponse response, Model model) {
		if (user.getSchool()==null || user.getSchool().getId()==null || "".equals(user.getSchool().getId()))
			user.setSchool(UserUtils.getUser().getSchool());
		if (user.getOffice()!=null && !"".equals(user.getOffice().getName()) && !"".equals(user.getOffice().getId()))
			user.setOffice(officeService.get(user.getOffice().getId()));
	...
	public String save(User user, HttpServletRequest request, Model model, RedirectAttributes redirectAttributes) {
		...
		// 新增用户保持跟操作人同属一个学校租户
		if (user.getSchool()==null || user.getSchool().getSchoolCode()==null || "".equals(user.getSchool().getSchoolCode()))
			user.setSchool(UserUtils.getUser().getSchool());
		...
		user.setRolenames(user.getRoleNamesN());
		//生成用户二维码，使用登录名
	...
	public String checkMobile(String oldtel, String mobile) {
		if (mobile !=null && mobile.equals(oldtel)) {
			return "true";
...
//				currentUser.setPhoto(user.getPhoto());
	...
	public String resetPass(User user, Model model) {
		...
		String newPass = user.getMobile(), resultStr="重置成功，重置后密码为：" + newPass;
		Random ra = new Random();
		if ("".endsWith(newPass) || newPass == null) {
			newPass = "" + (ra.nextInt(999999)%900000 + 100000);
			resultStr = "重置成功，重置后密码为：" + newPass;
		}
		systemService.updatePasswordById(user.getId(), user.getLoginName(), newPass);
	...
	public List<Map<String, Object>> treeDataCc(@RequestParam(required=false) String campusId, HttpServletResponse response) {
		List<Map<String, Object>> mapList = Lists.newArrayList();
		if (campusId != null && !"".equals(campusId))
		{
			List<User> list = systemService.findUserByCompanyId(campusId);
			for (int i=0; i<list.size(); i++){
				User e = list.get(i);
				Map<String, Object> map = Maps.newHashMap();
				map.put("id", "u_"+e.getId());
				map.put("pId", campusId);
				map.put("name", StringUtils.replace(e.getName(), " ", ""));
				mapList.add(map);
			}
		 
			Office ts = officeService.get(campusId);
			Map<String, Object> map = Maps.newHashMap();
			map.put("id", ts.getId());
			map.put("pId", "0");
