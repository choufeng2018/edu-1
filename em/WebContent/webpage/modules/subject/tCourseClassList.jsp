...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="TCourseClass" action="${ctx}/subject/tCourseClass/list" method="post" class="form-inline">
	...
	<div class="col-sm-12">
		<div class="pull-left">
			<shiro:hasPermission name="subject:tCourseClass:add">
				<table:addRow url="${ctx}/subject/tCourseClass/form" title="排课" width="900px" height="600px" label="排课"></table:addRow><!-- 增加按钮 -->
			</shiro:hasPermission><!-- 
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				...
				<th  class="sort-column slotdesc">上课时段</th>
		...
		<c:forEach items="${page.list}" var="tCourseClass">
			<tr>
				<!-- <td> <input type="checkbox" id="${tCourseClass.id}" name="${tCourseClass.iscreate}" class="i-checks"></td> -->
				<c:if test="${UserUtils.getUserIsParent()}">
				<td>
				...
				<td>
					...
					<c:if test="${tCourseClass.status == '1'}">
					<shiro:hasPermission name="subject:tCourseClass:edit">
    					<a href="#" onclick="openDialog('修改课程方案', '${ctx}/subject/tCourseClass/form?id=${tCourseClass.id}','800px', '600px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 修改</a>
    				</shiro:hasPermission>
    				<shiro:hasPermission name="subject:tCourseClass:del">
						<a href="${ctx}/subject/tCourseClass/delete?id=${tCourseClass.id}" onclick="return confirmx('确认要删除该课程方案吗？', this.href)"   class="btn btn-danger btn-xs"><i class="fa fa-trash"></i> 删除</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="subject:tCourseClass:check">
						<a href="${ctx}/subject/tCourseClass/check?id=${tCourseClass.id}" class="btn btn-info btn-xs"><i class="fa fa-search-plus"></i> 开始招生</a>
					</shiro:hasPermission>
    				</c:if>
    				<c:if test="${tCourseClass.iscreate==0}">
						<shiro:hasPermission name="subject:tCourseTimetable:add">
							<a href="${ctx}/subject/tCourseClass/createTimeTable?ids=${tCourseClass.id}" class="btn btn-success btn-xs" ><i class="fa fa-search-plus"></i> 创建课程表</a>
						</shiro:hasPermission>
					</c:if>
					<c:if test="${tCourseClass.status<3 && tCourseClass.iscreate==1}"></c:if>
					<shiro:hasPermission name="subject:tCourseTimetable:add">
					...
					<c:if test="${tCourseClass.iscreate == 1}">
						<a href="#" onclick="openDialogView('课程表', '${ctx}/subject/tCourseTimetable/subjectlist?ccid=${tCourseClass.id}','900px', '600px')" class="btn btn-success btn-xs" ><i class="fa fa-search-plus"></i> 查看课程表</a>
    				</c:if>
    				-->
				</td>
			</tr>
