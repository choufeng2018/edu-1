...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
	<form:form id="searchForm" modelAttribute="TClassStudent" action="${ctx}/teaching/tClassStudent/todoleavelist" method="post" class="form-inline">
		...
		<div class="form-group">
			...
			<span>授课方式：</span>
				<form:select path="courseclass.classType"  class="form-control m-b">
					<form:option value="" label=""/>
					<form:options items="${fns:getDictList('course_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				</form:select>
				<!--
				<form:radiobuttons class="i-checks" path="courseclass.classType" items="${fns:getDictList('course_type')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
				-->
		 </div>	
	</form:form>
	<br/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		...
		<c:forEach items="${page.list}" var="tClassStudent">
			<tr>
				...
				<td>
					<c:if test="${tClassStudent.courseclass.classType == 1}">
						<shiro:hasPermission name="teaching:tClassStudent:edit">
	    					<a href="#" onclick="openDialogParent('课时调整', '${ctx}/teaching/tClassStudentCheck/leaveform?tCourseTimetable.courseclass.id=${tClassStudent.courseclass.id}&studentId=${tClassStudent.student.id}','900px', '500px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 课时调整</a>
	    				</shiro:hasPermission>
					</c:if>
					<shiro:hasPermission name="teaching:tClassStudent:edit">
    					<a href="#" onclick="openDialogParent('请假', '${ctx}/teaching/tClassStudentCheck/leaveform?tCourseTimetable.courseclass.id=${tClassStudent.courseclass.id}&studentId=${tClassStudent.student.id}','900px', '500px')" class="btn btn-success btn-xs" ><i class="fa fa-edit"></i> 请假</a>
    				</shiro:hasPermission>
				</td>
			</tr>
		</c:forEach>
