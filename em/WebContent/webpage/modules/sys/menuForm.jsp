...
<head>
	...
	<script type="text/javascript">
		...
		function doSubmit(){//回调函数，在编辑和保存动作时，供openDialog调用提交表单。
			if ($('input[name="isShow"]:checked').val()=="0" && $('input[name="isResource"]:checked').val()=="1")
			{
				alert("不可见菜单不能作为资源"); return false;
			}
			if ($("#href").val()=="" && $('input[name="isResource"]:checked').val()=="1")
			{
				alert("链接为空菜单不能作为资源"); return false;
			}
		  	if($('input[name="isResource"]:checked').val()=="1"){
			  	return confirmx('确认要将该菜单添加为系统租户资源吗？', doSubmitCon);
		  	} else 
				return doSubmitCon();
		}
		function doSubmitCon(){//回调函数，在编辑和保存动作时，供openDialog调用提交表单。
	...
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal">
		...
		<table class="table table-bordered  table-condensed dataTables-example dataTable no-footer">
		   <tbody>
		      ...
		      <tr>
		      	 <td  class="width-15 active"><label class="pull-right">是否为租户资源：</label></td>
		         <td  class="width-35" >
		         	<form:radiobuttons path="isResource" items="${fns:getDictList('is_resource')}" itemLabel="label" itemValue="value" htmlEscape="false" class="required i-checks "/>
		         	<span class="help-inline">链接不为空、可见的菜单才能作为资源</span>
		         </td>
