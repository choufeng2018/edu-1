...
<%@ attribute name="queryparam" type="java.lang.String" required="false" description="前置校验属性不能为空"%>
	...
	$("#${id}Button, #${id}Name").click(function(){
		...
		if (${needcheck}) {
			if (validata)
			{
				...
				if ("${queryparam}"!="")
				{
					var obj1 = validata("${queryparam}", "学科");
					if (obj1.tag == "0")
					{
						top.layer.msg(obj1.str, {icon: 0});
						return true;
					} else {
						var ind = param.indexOf("?");
						if (obj1.str!="" && obj1.str!=null)
							param = param + (ind>0?"&":"?") + obj1.str;
					}
				}
			} else {
		...
		//var param = encodeURIComponent("${url}") + "${checkparam}=" + 
		...
		top.layer.open({
		    ...
		    content: "${ctx}/tag/treeselect?url="+encodeURIComponent(param)+"&module=${module}&checked=${checked}&extId=${extId}&isAll=${isAll}" ,
		    btn: ['确定', '关闭']
