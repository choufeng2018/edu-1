...
function searchGrid${id}(){
	...
	top.layer.open({
	    type: 2,  
	    area: ["${width}"==""?'800px':"${width}", "${height}"==""?'500px':"${height}"],
	    title:"${title}",
	    name:'friend',
	    //content: "${url}&fieldLabels=${fieldLabels}&fieldKeys=${fieldKeys}&url=${url}&searchLabel=${searchLabel}&searchKey=${searchKey}" ,
	    content: ind>0?"${url}&fieldLabels=${fieldLabels}&fieldKeys=${fieldKeys}":"${url}?fieldLabels=${fieldLabels}&fieldKeys=${fieldKeys}" ,
	    btn: ['确定', '关闭'],
	    yes: function(index, layero){
	    	 ...
	    	 if ("${callback}"=="1" && (typeof setValue === 'function'))
	    	 {
	    	 	setValue(item);
	    	 }
